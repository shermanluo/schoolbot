...
module.exports = function(controller) {
            ...
            convo.ask("Please type the name of the class this submission is for",  function(response, convo) {
                ...
                assignmentname = response.text; 
                    convo.ask("Please type the public URL of the file you wish to send", function(response, convo) {
                        //db.addFileURL(message.user, Y2lzY29zcGFyazovL3VzL1JPT00vOGFlYWExZDAtYWI5Yy0xMWU3LWEyOWItMmRlMTAyNTBkMjRm, assignmentname, response.text);
                        url = response.text
    ...
    controller.hears(['^check submission(.*)'], 'direct_message,direct_mention', function(bot, message) {
        bot.startConversation(message, function(err, convo) {
            var classname = ""; 
            bot.ask("Please type the name of the class you would like to check", function(response, convo) {
                classname = response.text; 
                var assignmentname = ""; 
            ...
            //if (db.getFileURL(message.user, assignmentname)) {
                  //returnurl = db.getFileURL(message.user, Y2lzY29zcGFyazovL3VzL1JPT00vOGFlYWExZDAtYWI5Yy0xMWU3LWEyOWItMmRlMTAyNTBkMjRm, assignmentname); 
            //} else {
